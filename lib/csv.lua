...
local function trim_space(s) return s:match("^%s*(.-)%s*$") end
...
function column_map:new(columns)
    ...
    for n, v in pairs(columns) do
        ...
        if not names then names = {(n:lower():gsub("[^%w%d]+", " "))} end

        t.name = n
        for _, n in ipairs(names) do name_map[n:lower()] = t end
    end
...
function column_map:transform(value, index)
    ...
    if field then
        if field.transform then
            ...
            if not ok then
                error(
                    ("Error reading field '%s': %s"):format(field.name, value),
                    0)
            end
        end
...
function file_buffer:find(pattern, init)
    while true do
        local first, last, capture = self.buffer:find(pattern,
                                                      init - self.buffer_start)
        ...
        if not first or last == #self.buffer then
            ...
            if not s then
                if not first then
                    ...
                    return first + self.buffer_start, last + self.buffer_start,
                           capture
                end
            end
...
local function find_unicode_BOM(sub)
    for _, x in ipairs(unicode_BOMS) do
        ...
        if x.BOMS[code] then return x.length end
    end
    return 0
end
...
local function separated_values_iterator(buffer, parameters)
    ...
    if buffer.truncate then
        ...
        advance = function(n) field_start = field_start + n end
    end
    ...
    local function problem(message)
        error(("%s:%d:%d: %s"):format(parameters.filename, field_start_line,
                                      field_start_column, message), 0)
    end
        ...
        if not this_sep or this_sep == "\r" or this_sep == "\n" then
            ...
            elseif parameters.header and not header_read then
                ...
                if nonblanks or field_count > 1 then -- ignore blank lines
                    ...
                    if parameters.record_limit and record_count >=
                        parameters.record_limit then break end
                end
            end
...
local buffer_mt = {
    ...
    close = function(t) if t.buffer.close then t.buffer:close() end end,
    name = function(t) return t.parameters.filename end
}
...
local function use(buffer, -- ?string|file|buffer: the buffer to read from.  If it's:
...
--  @return a file object
local function open(filename, -- string: name of the file to open
parameters) -- ?table: parameters controlling reading the file.
    -- See README.md
...
--  @return a file object
local function openstring(filecontents, -- string: The contents of the delimited file
                          parameters) -- ?table: parameters controlling reading the file.
    -- See README.md
