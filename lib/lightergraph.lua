...
local function graph_to_screen(self, x, y, round)
    if self._x_warp == "exp" then
        x = util.explin(self._x_min, self._x_max, self._x,
                        self._x + self._w - 1, x)
    else
        x = util.linlin(self._x_min, self._x_max, self._x,
                        self._x + self._w - 1, x)
    end
    if self._y_warp == "exp" then
        y = util.explin(self._y_min, self._y_max, self._y + self._h - 1,
                        self._y, y)
    else
        y = util.linlin(self._y_min, self._y_max, self._y + self._h - 1,
                        self._y, y)
    end
    if round then x, y = util.round(x), util.round(y) end
    return x, y
end

local function recalculate_screen_coords(self)
    self.origin_sx = util.round(util.linlin(self._x_min, self._x_max, self._x,
                                            self._x + self._w - 1, 0))
    self.origin_sy = util.round(util.linlin(self._y_min, self._y_max,
                                            self._y + self._h - 1, self._y, 0))
    for i = 1, #self._points do
        self._points[i].sx, self._points[i].sy =
            graph_to_screen(self, self._points[i].x, self._points[i].y, true)
    end
...
local function generate_line_from_points(self)
    ...
    if #self._points < 2 or
        (self._style ~= "line" and self._style ~= "line_and_point") then
        return
    end
    ...
    for i = 2, #self._points do
        ...
        if curve == "exp" or
            (type(curve) == "number" and math.abs(curve) > 0.01) then
            ...
            if sx_distance <= 1 or prev_sy == sy then
                ...
                for sample_x = prev_sx + 1, sx - 1 do
                    ...
                    if self._x_warp == "exp" then
                        local sample_graph_x =
                            util.linexp(self._x_min, self._x_max, self._x_min,
                                        self._x_max, prev_px + (px - prev_px) *
                                            sample_x_progress)
                        local prev_px_exp =
                            util.linexp(self._x_min, self._x_max, self._x_min,
                                        self._x_max, prev_px)
                        local px_exp = util.linexp(self._x_min, self._x_max,
                                                   self._x_min, self._x_max, px)
                        sample_x_progress =
                            (sample_graph_x - prev_px_exp) /
                                (px_exp - prev_px_exp)
                    end
                    if sample_x_progress <= 0 then
                        sample_x_progress = 1
                    end
                    ...
                    if curve == "exp" then
                        ...
                        sy_section = util.linexp(0, 1, prev_adj_y, cur_adj_y,
                                                 sample_x_progress)

                    else
                        -- Curve formula from SuperCollider
                        sy_section = util.linlin(0, 1, prev_py, py, a -
                                                     (a *
                                                         math.pow(grow,
                                                                  sample_x_progress)))
                    ...
                    if self._y_warp == "exp" then
                        sy_section = util.explin(self._y_min, self._y_max,
                                                 self._y + self._h - 1, self._y,
                                                 sy_section)
                    else
                        sy_section = util.linlin(self._y_min, self._y_max,
                                                 self._y + self._h - 1, self._y,
                                                 sy_section)
                    end
...
local function generate_lines_from_functions(self)
    ...
    for i = 1, #self._functions do
        ...
        for sx = self._x, self._x + width, step do
            local x, y
            if self._x_warp == "exp" then
                x = util.explin(self._x, self._x + width, self._x_min,
                                self._x_max, sx)
            else
                x = util.linlin(self._x, self._x + width, self._x_min,
                                self._x_max, sx)
            end
...
local function generate_spline_from_points(self)
    ...
    for i = 1, num_points - 1 do
        table.insert(one_thirds, interpolate_points(points[i], points[i + 1],
                                                    0.333333, exp_x, exp_y))
        table.insert(two_thirds, interpolate_points(points[i], points[i + 1],
                                                    0.666666, exp_x, exp_y))
    end

    -- Create bezier coords
    for i = 1, num_points - 3 do
        table.insert(self._spline, interpolate_points(two_thirds[i],
                                                      one_thirds[i + 1], 0.5,
                                                      exp_x, exp_y)) -- Start
        table.insert(self._spline, one_thirds[i + 1])
        table.insert(self._spline, two_thirds[i + 1])
        table.insert(self._spline, interpolate_points(two_thirds[i + 1],
                                                      one_thirds[i + 2], 0.5,
                                                      exp_x, exp_y))
    end
...
function Graph.new(x_min, x_max, x_warp, y_min, y_max, y_warp, style,
...
function Graph:set_active(bool) self._active = bool == nil and false or bool end
...
function Graph:get_point(index) return self._points[index] end
...
function Graph:edit_point(index, px, py, curve, highlight)
    ...
    if px or py then
        self._points[index].sx, self._points[index].sy =
            graph_to_screen(self, self._points[index].x, self._points[index].y,
                            true)
    end
...
function Graph:highlight_point(index) self._points[index].highlight = true end
...
function Graph:clear_highlight(index) self._points[index].highlight = false end

--- Clear highlights from all points.
function Graph:clear_all_highlights()
    for i = 1, #self._points do self._points[i].highlight = false end
end
...
function Graph:get_function(index) return self._functions[index].func end
...
function Graph:add_function(func, sample_quality, index)
    ...
    if index then
        table.insert(self._functions, index,
                     {func = func, sample_quality = quality})
    else
...
function Graph:edit_function(index, func, sample_quality)
    ...
    if sample_quality then
        self._functions[index].sample_quality = sample_quality
    end
    self._lines_dirty = true
end
...
function Graph:update_functions() self._lines_dirty = true end
...
local function draw_points(self)
    ...
    if (self._style ~= "point" and self._style ~= "line_and_point" and
        self._style ~= "spline_and_point") then return end
    ...
    for i = 1, #self._points do
        ...
        if self._points[i].highlight then
            if sy < self.origin_sy then
                ...
                screen.rect(sx - 1, self.origin_sy, 3,
                            math.max(1, sy - self.origin_sy + 1))
            end
            if self._active then
                screen.level(15)
            else
            ...
            elseif sy < self.origin_sy then
                screen.rect(sx - 0.5, sy + 0.5, 2,
                            math.max(0, self.origin_sy - sy))
                screen.stroke()
            else
                screen.rect(sx - 0.5, self.origin_sy + 0.5, 2,
                            math.max(0, sy - self.origin_sy))
                screen.stroke()
            end
        end
...
local function draw_lines(self)
    ...
    if (self._style ~= "line" and self._style ~= "line_and_point") and
        #self._functions == 0 then return end
...
local function draw_spline(self)
    if (self._style ~= "spline" and self._style ~= "spline_and_point") then
        return
    end
    ...
    for i = 1, #self._spline - 4, 4 do
        ...
        screen.curve(self._spline[i + 1].x, self._spline[i + 1].y,
                     self._spline[i + 2].x, self._spline[i + 2].y,
